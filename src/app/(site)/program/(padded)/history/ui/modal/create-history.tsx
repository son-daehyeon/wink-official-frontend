...
import { getProgramUploadUrl, useCreateHistoryMutation } from '@/features/program';
...
export default function CreateHistoryModal({ open, setOpen, callback }: CreateHistoryModalProps) {
  ...
  const onSubmit = useCallback(
    (values: CreateHistoryRequest) => {
      startApi(
        async () => {
          const { history } = await createHistory.mutateAsync(values);
          callback(history);
        },
        {
    ...
    [callback, createHistory, form, setOpen, startApi],
  );
