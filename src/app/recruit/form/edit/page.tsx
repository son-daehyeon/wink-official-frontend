...
export default function RecruitApplicationPage() {
  ...
  const editToken = useMemo(() => params.get('token'), [params]);
  ...
  useEffect(() => {
    ...
    startApi(async () => {
      const { form: storedForm } = await Api.Domain.Recruit.getEditForm(editToken);

      form.reset(storedForm);
      setDeveloper(
        !!(
          storedForm.github &&
      ...
      setStack([
        ...
        ...(storedForm.devOpsTechStacks.length > 0 ? ['frontend'] : []),
        ...(storedForm.designTechStacks.length > 0 ? ['design'] : []),
      ] as Stack[]);
      setStep(18);
      setEditToken(editToken);
      setRecruit(storedForm.recruit);
    });
  }, [editToken]);
  ...
  if (isApi || !editToken || !recruit) return <Loading />;
