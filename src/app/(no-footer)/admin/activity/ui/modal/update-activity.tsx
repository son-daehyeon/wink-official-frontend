...
import { CreateActivityRequest, CreateActivityRequestSchema } from '@/entities/program';
import { Activity } from '@/entities/program';
import { updateAdminActivity, uploadAdminProgramImage } from '@/features/admin';
...
export default function UpdateActivityModal({
  ...
  const onSubmit = useCallback(
    async (values: CreateActivityRequest) =>
      startApi(
        async () => {
          const { activity: _activity } = await updateAdminActivity(activity!.id, values);
          callback(_activity);
        },
        {
    ...
    [activity, callback, form, setOpen, startApi],
  );
                      ...
                      <input
                        ...
                        onChange={async (e) =>
                          startUpload(
                            async () =>
                              field.onChange(
                                await uploadS3(e.target.files!, () => uploadAdminProgramImage()),
                              ),
                            {
