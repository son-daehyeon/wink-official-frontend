...
import { useCheckStudentIdMutation } from '@/features/recruit';
  ...
  return (
    <>
      ...
      >
        <Button
          variant="wink"
          disabled={clicked || checkStudentIdMutation.isPending}
          onClick={async () => {
            setClicked(true);

            try {
              if (await form.trigger('studentId')) {
                const { duplicated } = await checkStudentIdMutation.mutateAsync({
