...
import { departments } from '../../../../recruit/form/_constant/departments';

import { UpdateRequest, UpdateRequestSchema } from '@/entities/user';
import { Role, getKoreanRole } from '@/entities/user';
...
export default function UpdateUserModal({ open, setOpen, user, callback }: UpdateUserModalProps) {
  ...
  const onSubmit = useCallback(
    (values: UpdateRequest) => {
      startApi(
        async () => {
          const { user: _user } = await updateAdminUser(user!.id, values);
          callback(_user);
        },
        {
    ...
    [callback, setOpen, startApi, user],
  );
