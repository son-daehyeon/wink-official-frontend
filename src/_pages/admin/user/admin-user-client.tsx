...
import InviteUserModal from './_component/modal/invite-user';
import UpdateUserModal from './_component/modal/update-user';

import { getKoreanRole } from '@/entities/user';
...
export default function AdminUserPage() {
  ...
  const usersQuery = useQuery(adminQueryOptions.users(browserApi, page, query));
  const [usersOverride, setUsersOverride] = useState<Page<User>>();
  ...
  const onUpdateUser = useCallback(
    (user: User) => {
      setUsersOverride((prev) => {
        ...
        return { ...current, content: current.content.map((u) => (u.id === user.id ? user : u)) };
      });
    },
    [users],
  );
