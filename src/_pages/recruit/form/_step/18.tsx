...
import ConfirmSurveyModal from '../_component/modal/confirm-survey';
...
export default function Step18({ go, recruit, form }: RecruitStepProps) {
  const { setModify, developer, stack, editing } = useRecruitStore();
  ...
  return (
    <>
      ...
      >
        <p className="font-medium text-lg">지원서를 {editing ? '수정' : '제출'}할까요?</p>
      </motion.div>
            ...
            >
              ...
              <TableCell className="whitespace-pre-line break-all hover:underline cursor-pointer">
                {form
                  ...
                  .map((date) =>
                    date === '0001-01-01' ? ALTERNATE_INTERVIEW_REQUIRED : formatDate(date, true),
                  )
                  .join('\n')}
              </TableCell>
            </TableRow>
