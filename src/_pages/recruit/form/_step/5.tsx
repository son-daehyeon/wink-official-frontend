...
import { useCheckPhoneNumberMutation } from '@/features/recruit';
...
export default function Step5({ go, recruit, form }: RecruitStepProps) {
  ...
  const checkPhoneNumberMutation = useCheckPhoneNumberMutation();
  ...
  return (
    <>
      ...
      >
        <Button
          variant="wink"
          disabled={clicked || checkPhoneNumberMutation.isPending}
          onClick={async () => {
            setClicked(true);

            try {
              if (await form.trigger('phoneNumber')) {
                const { duplicated } = await checkPhoneNumberMutation.mutateAsync({
