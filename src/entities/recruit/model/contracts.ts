import type { RecruitFormSummary } from './admin-recruit-form';
import type Recruit from './recruit';
import type RecruitForm from './recruit-form';
import {
  BackendTechStack,
  DesignTechStack,
  DevOpsTechStack,
...
import type RecruitSms from './recruit-sms';

import { VALID_DEPARTMENTS } from '@/entities/department/@x/recruit';
import {
...
export const PRIVACY_POLICY_VERSION = '2026-07-29' as const;
export const ALTERNATE_INTERVIEW_REQUIRED = '별도 일정 조율 필요' as const;
...
export const RecruitFormRequestSchema = z
    ...
    outings: z
      .array(
        z
          .string()
          .min(1, '대외활동을 입력해주세요.')
          .max(100, '대외활동은 항목당 100자 이하이어야 합니다.'),
      )
      .max(20, '대외활동은 최대 20개까지 입력할 수 있습니다.'),
    interviewDates: z
      .array(z.string().regex(YYYY_MM_DD_EXPRESSION, YYYY_MM_DD_MESSAGE))
      .min(1, '면접 날짜를 선택해주세요.'),
    whyCannotInterview: z.string().max(200, '대체 일정은 200자 이하이어야 합니다.').optional(),
    github: z
      .string()
  ...
  .superRefine((data, ctx) => {
    if (
      data.interviewDates.includes('0001-01-01') &&
      data.whyCannotInterview !== ALTERNATE_INTERVIEW_REQUIRED
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['whyCannotInterview'],
        message: '별도 일정 조율 필요 여부를 선택해주세요.',
      });
    }
  });
...
export type EditableRecruitForm = Omit<
  RecruitForm,
  'favoriteProject' | 'github' | 'whyCannotInterview'
> & {
...
export const GetFormResponseSchema = z.object({
  form: z.custom<EditableRecruitForm>(),
});

export const GetFormsResponseSchema = z.object({
  forms: z.array(z.custom<RecruitFormSummary>()),
});
...
export function createRecruitFormDefaultValues(): RecruitFormRequest {
  return {
    name: '',
    studentId: '',
    department: '',
